...
const eventData =[
 ...
 eventContainer.classList.add('event-flex');
 
 for (let i = 0; i < 3; i++) {
   const eventDiv = document.createElement('div');
   eventDiv.classList.add('event-item');
 
   createEventContent(eventDiv, eventData[i]);
 ...
 function createEventContent(eventDiv, eventData) {
   const imageElement = document.createElement('img');
   imageElement.src = eventData.imageUrl;
   imageElement.alt = eventData.title;
   ...
   priceElement.textContent = `$${eventData.price}`;
   ...
   companyElement.textContent = `Hosted by: ${eventData.company}`;
